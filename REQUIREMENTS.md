...
> Status legend: ✅ shipped · ◐ partial (honest scope cut documented) · ⬜ not started. Last build pass: 2026-07-08, batches A-D.
...
| R3 ✅ (S8) | **Rule-length / info-density lint**: imperative rules >~1 sentence are observably ignored; flag verbose rules and decoration-heavy entries. | HN 48160604, 47144537, mem0#4573; hermes#47349 (70+ entries, 14K chars burned per turn), openclaw#92451 (instruction-count regression dilutes attention) |

## v0.2 (L-tier + U-tier — the differentiators)

| id | Requirement | Evidence |
|----|-------------|----------|
| R4 ✅ (L1, mock-tested; real-data run pending API key) | **Contradiction / supersession detector** (L1, already designed — evidence now overwhelming): embed-cluster same-subject entries, judge-classify duplicate vs conflicting; also flag "newer same-subject fact exists but old one still loads". | mem0 ×4, graphiti#630/#1166, HN ×4; hermes#509 ("PostgreSQL Monday, MySQL Friday, both persist forever"), #346; openclaw#82339 (divergent per-workspace copies) |
| R5 ✅ (L1 classification) | **Junk-content classifier**: system-prompt restatement / cron noise / transient task state / architecture dump vs durable fact. The 97.8% stat is the demo. | mem0#4573 (audited), #2736, HN "earn its keep" |
| R6 ◐ (evidence plumbing + `compliance` cmd shipped; violation judging = future --llm pass) | **Compliance-gap audit (U-tier flagship)**: extract MUST/NEVER rules from memory, correlate with local session transcripts for violations → "loaded ≠ obeyed; this rule isn't memory-enforceable, make it a hook." Claude Code JSONL transcripts make this local-first feasible. | claude-code#2544/#33603/#2142/#53223, codex#23496/#4466; openclaw#87857 (mandated startup reads skipped by runtime) |
| R7 ✅ (U2 frequency audit; vanish-check consciously NOT shipped — data can't attribute to a boundary) | **Compaction-boundary audit**: fingerprint the active instruction set before/after compaction events in session logs; flag rules/facts that vanish at the boundary. Compaction is the #1 memory-fidelity destroyer in both CLIs. | claude-code#9796/#13919/#24179, codex#5957/#25792, letta#3270/#3242; openclaw#3922/#97545 ("dementia effect"), #45608 (reset without memory flush); hermes#5563 (post-session env-fact drift) |
| R8 ✅ (L1) | **Over-generalization flag**: absolute rules citing a single dated incident, no scope/expiry ("don't use Stripe" said once → permanent bias). | HN 47900726; new taxonomy class |

## v0.3 (framework adapters — Mem0 first, the junk-audit demo)

| id | Requirement | Evidence |
|----|-------------|----------|
| R9 ◐ (M1 export-visible half, experimental) | **Cross-store ghost check**: entry deleted in one backing store but alive in another (vector vs graph vs entity table) still surfaces in search. | mem0 ×6 issues (#3695, #4869, #3245, #5577, #4863, #2165), letta#2237 |
| R10 ◐ (M2 subject-conflict lite, experimental) | **Cross-file/store consistency**: same fact, different values across files/stores (SQL vs vector drift; CLAUDE.md vs AGENTS.md vs project memory duplication). | HN 46205076, claude-code#34235 family, mem0#3371 |
| R11 ⬜ (needs a live store — honest stub, see README roadmap) | **Round-trip / configured-but-dead probes**: write via each write path, confirm retrievability via each read path; flag configured-but-zero-calls stores and embedder-drift (memories stored under a different embedding model = unretrievable). | langmem#140/#138/#114, letta#3210 |
| R12 ✅ (U1 + ≥24h window gate) | **Never-retrieved-in-N-days + feedback-loop amplification** (recalled memories re-extracted as new duplicates → runaway growth). | mem0#5330 ×5, #4573; hermes#37447 (community-built 14-day decay scoring), #4379 (73% fixed overhead), #17649 (98% skill-load waste → load-vs-use ratio) |

## New in the hermes/openclaw pass (graduated)

| id | Requirement | Evidence |
|----|-------------|----------|
| R13 ◐ (near-cliff static half in S1; reset-without-flush = future) | **Silent write-loss audit**: memory files at/near capacity limits where new writes silently fail or resets skip the flush — "operator corrections silently lost". Detect at-capacity files + reset-events with no preceding memory write. | hermes#32064, openclaw#45608, letta#3151 (3 systems) |
| R14 ⬜ (v0.4 fleet mode) | **Scope-leakage check (fleet mode)**: one shared memory file written by multiple users/agents without scoping — cross-user contamination; unscoped-write onboarding patterns. Maps to the boundary-lab "wrong-scope" taxonomy class. | hermes#39492, gbrain#2660 |
| R15 ✅ (S7) | **Staleness-by-neglect (mtime audit, S-tier — cheap, add to v0.1.x)**: instruction/persona files never modified since creation while memory keeps growing (SOUL.md static forever); in-memory caches never refreshed vs disk mtime. Pure timestamp check. | hermes#11919, openclaw#26497 |
| R16 ◐ (S4 covers claude-code; other systems' bridges = adapter-era) | **Index-vs-disk integrity (S4 extension)**: memory index/bridge reports zero or fewer artifacts than exist on disk — memory present but invisible to the system. | openclaw#66082/#65722/#85655 (3 dup issues), langmem#140 |
